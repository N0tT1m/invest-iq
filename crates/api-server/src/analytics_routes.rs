use analytics::{PerformanceOverview, SignalQuality, SignalQualityReport, StrategyPerformance};
use axum::{
...
pub fn analytics_routes() -> Router<AppState> {
    Router::new()
        ...
        .route(
            "/api/analytics/strategy/:name",
            get(get_strategy_performance),
        )
        .route("/api/analytics/top/:limit", get(get_top_strategies))
        .route(
            "/api/analytics/performance/update",
            post(update_performance),
        )
        // Signal quality endpoints
        .route(
            "/api/analytics/signals/quality",
            get(get_signal_quality_report),
        )
        ...
        .route(
            "/api/analytics/signals/calibrate",
            get(calibrate_confidence),
        )
}
...
async fn update_performance(
    ...
    tracker
        .update_strategy_performance(&req.strategy_name, &req.symbol, req.is_win, req.profit_loss)
        .await?;

    Ok(Json(ApiResponse::success(
        "Performance updated".to_string(),
    )))
}
...
async fn record_signal_outcome(
    ...
    analyzer
        .record_trade_outcome(&req.signal_type, req.confidence, req.is_win, req.return_pct)
        .await?;

    Ok(Json(ApiResponse::success(
        "Signal outcome recorded".to_string(),
    )))
}
...
async fn check_signal_filter(
    ...
    let should_filter = analyzer
        .should_filter_signal(&query.signal_type, query.confidence, min_win_rate)
        .await?;
...
async fn calibrate_confidence(
    ...
    let calibrated = analyzer
        .get_calibrated_confidence(&query.signal_type, query.confidence)
        .await?;
