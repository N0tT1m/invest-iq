...
pub fn short_interest_routes() -> Router<AppState> {
    Router::new().route("/api/short-interest/:symbol", get(get_short_interest))
}

#[utoipa::path(
    get,
    path = "/api/short-interest/{symbol}",
    params(("symbol" = String, Path, description = "Stock ticker symbol")),
    responses((status = 200, description = "Short squeeze risk analysis with 6-component scoring")),
    tag = "Market Data"
)]
async fn get_short_interest(
    ...
    // 1. Volume Spike (15%)
    let vol_spike_score = volume_spike
        .map(|v| ((v - 1.0) * 15.0).clamp(0.0, 15.0))
        .unwrap_or(0.0);
    components.push(SqueezeComponent {
        ...
        max_score: 15.0,
        description: format!(
            "Latest volume vs 20d avg: {:.1}x",
            volume_spike.unwrap_or(0.0)
        ),
    });

    // 2. Volume Trend (15%)
    let vol_trend_score = volume_trend
        .map(|v| ((v - 0.8) * 25.0).clamp(0.0, 15.0))
        .unwrap_or(0.0);
    components.push(SqueezeComponent {
    ...
    // 3. Momentum (20%)
    let momentum_score = recent_return
        .map(|r| if r > 0.0 { (r * 2.0).min(20.0) } else { 0.0 })
        .unwrap_or(0.0);
    components.push(SqueezeComponent {
    ...
    let vol_comp_score = volatility_percentile
        .map(|p| ((100.0 - p) / 100.0 * 15.0).clamp(0.0, 15.0))
        .unwrap_or(
            volatility
                .map(|v| ((40.0 - v.max(0.0)) / 40.0 * 15.0).clamp(0.0, 15.0))
                .unwrap_or(0.0),
        );
    components.push(SqueezeComponent {
        ...
        description: format!(
            ...
            if volatility_percentile.unwrap_or(50.0) < 25.0 {
                "Compressed"
            } else if volatility_percentile.unwrap_or(50.0) < 50.0 {
                "Below avg"
            } else {
                "Normal/Elevated"
            }
    ...
    let rsi_score = rsi
        .map(|r| {
            if r < 30.0 {
                ((30.0 - r) / 30.0 * 15.0).min(15.0)
            } else if r > 70.0 {
                ((r - 70.0) / 30.0 * 15.0).min(15.0)
            } else {
                0.0
            }
        })
    ...
    let squeeze_score =
        vol_spike_score + vol_trend_score + momentum_score + vol_comp_score + bb_score + rsi_score;
    ...
    if volume_spike.unwrap_or(0.0) > 1.5 {
        interp_parts.push(format!(
            "Volume is elevated at {:.1}x average, signaling increased trader interest.",
            volume_spike.unwrap_or(0.0)
        ));
    }
    if let Some(r) = rsi {
        if r < 30.0 {
            interp_parts.push(format!(
                "RSI at {:.1} shows oversold conditions, potential for short squeeze.",
                r
            ));
        } else if r > 70.0 {
            interp_parts.push(format!("RSI at {:.1} shows overbought conditions.", r));
        }
    }
    let interpretation = if interp_parts.is_empty() {
        format!(
            "Squeeze risk is {} with a score of {:.0}/100. No extreme conditions detected.",
            squeeze_level.to_lowercase(),
            squeeze_score
        )
    } else {
        interp_parts.join(" ")
