...
impl SignalAnalyzer {
    ...
    pub async fn get_all_signal_qualities(&self) -> Result<Vec<SignalQuality>> {
        let qualities: Vec<SignalQuality> = sqlx::query_as(
            "SELECT * FROM signal_quality ORDER BY actual_win_rate DESC"
        )
        .fetch_all(&self.pool)
        .await?;
    ...
    pub async fn get_quality_report(&self) -> Result<SignalQualityReport> {
        ...
        let mut best_signals: Vec<SignalQuality> = all_signals.iter()
            .filter(|s| s.signals_taken >= 10) // Minimum sample size
            .filter(|s| s.actual_win_rate >= 0.60) // 60%+ win rate
            .cloned()
            .collect();
        best_signals.sort_by(|a, b| {
            b.actual_win_rate.partial_cmp(&a.actual_win_rate).unwrap()
        });
        best_signals.truncate(10);

        // Get worst signals (low win rate)
        let mut worst_signals: Vec<SignalQuality> = all_signals.iter()
            .filter(|s| s.signals_taken >= 10)
            .cloned()
            .collect();
        worst_signals.sort_by(|a, b| {
            a.actual_win_rate.partial_cmp(&b.actual_win_rate).unwrap()
        });
