...
pub fn dividend_routes() -> Router<AppState> {
    Router::new()
        .route("/api/dividends/:symbol", get(get_dividends))
}
...
async fn get_dividends(
    ...
    let dividends = state.orchestrator.get_dividends(&symbol).await.unwrap_or_default();

    if dividends.is_empty() {
        // Fetch news for dividend-related headlines
        let articles = state.orchestrator.get_news(&symbol, 20).await.unwrap_or_default();
        let dividend_news: Vec<DividendNewsItem> = articles.iter()
    ...
    let current_yield = if annual_dividend > 0.0 {
        if let Ok(bars) = state.orchestrator.get_bars(
            &symbol,
