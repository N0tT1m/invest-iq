...
use analysis_orchestrator::{
    AnalysisOrchestrator, ScreenerFilters, ScreenerResult, StockScreener, StockUniverse,
};
...
use backtest_engine::{
    BacktestConfig, BacktestDb, BacktestEngine as BtEngine, BacktestResult as BtResult,
    HistoricalBar, Signal as BtSignal,
};
use broker_trait::BrokerClient;
use chrono::{DateTime, Utc};
use dashmap::DashMap;
use portfolio_manager::{AlertManager, PortfolioDb, PortfolioManager, TradeLogger};
...
struct AppState {
    ...
    broker_client: Option<Arc<dyn BrokerClient>>,
    ...
    ml_provider: Option<Arc<dyn ml_client::MLProvider>>,
    ws_broadcast: Option<ws_routes::WsBroadcast>,
    notification: Option<Arc<notification_service::NotificationService>>,
}
...
pub(crate) async fn get_cached_etf_bars(
        ...
        .await
    {
        Ok(bars) => {
            state
                .etf_bar_cache
                .insert(cache_key, (Utc::now(), bars.clone()));
            bars
        }
        Err(e) => {
...
struct AnalyzeQuery {
    ...
    #[allow(dead_code)]
    cache_ttl: u64,
    /// Timeframe: 1m, 5m, 15m, 30m, 1h, 4h, 1w, 1M (default: Day1)
...
pub(crate) async fn get_cached_analysis(
    ...
    let cached: Option<CachedAnalysis> = match &state.cache {
        ...
        CacheBackend::Memory(map) => map.get(&cache_key).map(|entry| entry.clone()),
    };
    ...
    if let Some(cached) = cached {
        ...
        if age < cache_ttl {
            tracing::info!(
                "💾 [get_cached_analysis] cache hit for {} (age: {}s)",
                cache_key,
                age
            );
    ...
    tracing::info!(
        "🔍 [get_cached_analysis] cache miss for {}, running analysis",
        cache_key
    );
    ...
    match &state.cache {
        CacheBackend::Redis(conn) => {
            ...
            if let Ok(json_str) = serde_json::to_string(&cached_analysis) {
                let _: Result<(), _> = conn.set_ex(&cache_key, json_str, cache_ttl).await;
            }
        }
...
#[openapi(
    info(
        title = "InvestIQ API",
        version = "0.1.0",
        description = "Stock analysis platform with technical, fundamental, quantitative, and sentiment analysis engines. Includes backtesting, paper/live trading, risk management, and autonomous trading agent.",
        license(name = "MIT"),
    ),
    tags(
        (name = "Analysis", description = "Stock analysis endpoints — full multi-engine analysis, bars, ticker details"),
        (name = "Portfolio", description = "Portfolio management — positions, trades, alerts, watchlist, snapshots"),
        (name = "Trading", description = "Broker integration — account info, order execution, positions (Alpaca)"),
        (name = "Backtest", description = "Backtesting engine — run backtests, walk-forward, Monte Carlo"),
        (name = "Risk", description = "Risk management — parameters, circuit breakers, position sizing, risk radar"),
        (name = "Agent", description = "Autonomous trading agent — pending trades, analytics, approval workflow"),
        (name = "Screener", description = "Stock screening and symbol search"),
        (name = "Sentiment", description = "Sentiment analysis — velocity, history, social sentiment"),
        (name = "Market Data", description = "Market data panels — earnings, dividends, options, insiders, correlation, macro"),
        (name = "Strategies", description = "Strategy health monitoring — alpha decay, performance snapshots"),
        (name = "Tax", description = "Tax optimization — harvest opportunities, wash sales, lots"),
        (name = "Analytics", description = "Performance analytics — strategy performance, signal quality"),
        (name = "Admin", description = "Administrative endpoints — retention, audit verification"),
    ...
    components(
        schemas(
            analysis_core::Bar,
            analysis_core::SignalStrength,
            analysis_core::AnalysisResult,
            analysis_core::UnifiedAnalysis,
            analysis_core::Timeframe,
            backtest_routes::RunBacktestRequest,
            backtest_routes::BacktestSummary,
            backtest_routes::MonteCarloQuery,
        ),
    ),
    paths(
        health_check,
        ...
        backtest_routes::run_walk_forward,
        backtest_routes::get_monte_carlo,
        backtest_routes::get_all_backtests,
        backtest_routes::get_backtest,
        backtest_routes::delete_backtest,
        backtest_routes::get_backtest_trades,
        backtest_routes::get_backtests_by_strategy,
    ),
)]
struct ApiDoc;
...
async fn shutdown_signal() {
    ...
    // Hard exit safety net: if the process hasn't exited 10s after Ctrl+C,
    // force-terminate. This prevents hangs from leaked child processes or
    // stuck tokio tasks.
    std::thread::spawn(|| {
        std::thread::sleep(Duration::from_secs(10));
        eprintln!("[shutdown] Hard exit after 10s timeout");
        std::process::exit(0);
    });
}

/// Kill the frontend child process group and abort the supervisor task.
fn shutdown_frontend(
    ...
    #[cfg(unix)]
    if pid > 0 {
        // The child was started with setpgid(0,0) so it's a process group
        // leader.  Kill the whole group (negative PID = process group).
        tracing::info!("Sending SIGTERM to frontend process group (PID: {})", pid);
        unsafe {
            libc::kill(-(pid as i32), libc::SIGTERM);
        }

        // Give it 2 seconds, then escalate
        std::thread::sleep(Duration::from_secs(2));
        unsafe {
            if libc::kill(pid as i32, 0) == 0 {
                tracing::warn!("Frontend still alive, sending SIGKILL");
...
/// Uses eprintln! because this runs before tracing is initialized.
#[cfg(unix)]
fn kill_port(port: u16) {
    ...
    for attempt in 0..2 {
        ...
        if pids.is_empty() {
            if attempt > 0 {
                eprintln!("[desktop] Port {} is now free", port);
            }
            return;
        }
        ...
        let signal_name = if attempt == 0 { "SIGTERM" } else { "SIGKILL" };

        for &pid in &pids {
            eprintln!(
                "[desktop] Sending {} to PID {} on port {}",
                signal_name, pid, port
            );
            unsafe {
        ...
        // Wait for processes to die — check every 100ms for up to 2s
        for _ in 0..20 {
            ...
            let still_alive = check
                .map(|o| {
                    String::from_utf8_lossy(&o.stdout)
                        .split_whitespace()
                        .any(|s| s.parse::<u32>().ok().is_some_and(|p| p != my_pid))
                })
                .unwrap_or(false);
            if !still_alive {
                eprintln!("[desktop] Port {} is now free", port);
        ...
        if attempt == 0 {
            eprintln!(
                "[desktop] Port {} still occupied after SIGTERM, escalating to SIGKILL",
                port
            );
        }
    }
    ...
    eprintln!(
        "[desktop] WARNING: Could not free port {} after SIGTERM+SIGKILL",
        port
    );
}

#[cfg(not(unix))]
fn kill_port(_port: u16) {}

/// Run the API server. This is the main entry point, usable both from main()
/// and from the desktop app (which embeds the server directly).
pub async fn run_server() -> anyhow::Result<()> {
    // Guard: detect if we're being launched as a Python multiprocessing worker.
    // When PyO3 embeds Python, sys.executable points to THIS binary, so Python's
    // multiprocessing "spawn" method will launch a new copy of api-server.
    // Detect this and exit immediately to prevent an infinite cascade.
    {
        let args: Vec<String> = std::env::args().collect();
        let is_mp_worker = args.iter().any(|a| a.contains("multiprocessing"))
            || std::env::var("_PYTHON_MULTIPROCESSING_WORKER").is_ok();
        if is_mp_worker {
            eprintln!("[api-server] Detected multiprocessing worker spawn, exiting.");
            return Ok(());
        }
    }

    // Prevent Python/HuggingFace libraries from spawning new processes.
    // TOKENIZERS_PARALLELISM: disables the Rust tokenizers' parallelism that
    //   triggers multiprocessing on macOS.
    // OMP_NUM_THREADS: prevents OpenMP from forking threads that confuse PyO3.
    // These are safe no-ops if embedded-ml is not enabled.
    if std::env::var("TOKENIZERS_PARALLELISM").is_err() {
    ...
    let _sentry_guard = {
        ...
        if let Some(ref dsn) = dsn {
            println!("Sentry enabled (DSN: {}...)", &dsn[..dsn.len().min(30)]);
        }
        dsn.map(|dsn| {
            sentry::init((
                dsn,
    ...
    // Initialize OpenTelemetry tracer (only if OTEL_EXPORTER_OTLP_ENDPOINT is set)
    #[cfg(feature = "otel")]
    let _otel_guard = {
        ...
        if let Some(ref ep) = endpoint {
            println!("OpenTelemetry enabled, exporting to {}", ep);
        }
                ...
                .with_resource(opentelemetry_sdk::Resource::new(vec![
                    opentelemetry::KeyValue::new(
                        ...
                        std::env::var("OTEL_SERVICE_NAME")
                            .unwrap_or_else(|_| "investiq-api".to_string()),
                    ),
                ]))
                .build();
            let tracer = provider.tracer("investiq-api");
            opentelemetry::global::set_tracer_provider(provider);
            Some(tracer)
        })
    };

    // Build tracing subscriber — compose optional layers via cfg
    macro_rules! init_tracing {
        ($fmt_layer:expr) => {{
            ...
            // Each cfg branch adds its layer; Option<L> implements Layer when L does
            #[cfg(all(feature = "otel", feature = "sentry"))]
            {
                let otel_layer = _otel_guard
                    .as_ref()
                    .map(|tracer| tracing_opentelemetry::layer().with_tracer(tracer.clone()));
                let sentry_layer = sentry::integrations::tracing::layer();
                base.with(otel_layer).with(sentry_layer).init();
            }
            #[cfg(all(feature = "otel", not(feature = "sentry")))]
            {
                let otel_layer = _otel_guard
                    .as_ref()
                    .map(|tracer| tracing_opentelemetry::layer().with_tracer(tracer.clone()));
            ...
            #[cfg(all(not(feature = "otel"), feature = "sentry"))]
            {
                let sentry_layer = sentry::integrations::tracing::layer();
                base.with(sentry_layer).init();
            }
            #[cfg(not(any(feature = "otel", feature = "sentry")))]
            {
                base.init();
            }
        }};
    ...
    if use_json {
        init_tracing!(tracing_subscriber::fmt::layer().json());
    } else {
        init_tracing!(tracing_subscriber::fmt::layer());
    }
    ...
    let polygon_api_key = match read_secret("POLYGON_API_KEY") {
        Some(key) => key,
        None => {
            anyhow::bail!("POLYGON_API_KEY must be set in environment or .env file");
        }
    };
    ...
    let comparison_engine = std::env::var("ALPHA_VANTAGE_API_KEY").ok().map(|key| {
    ...
    let (
        ...
        performance_tracker,
        signal_analyzer,
    ) = match PortfolioDb::new(&database_url).await {
        Ok(db) => {
        ...
        Err(e) => {
            tracing::warn!(
                "⚠️  Failed to initialize portfolio database: {}. Portfolio features disabled.",
                e
            );
    ...
    // Initialize broker client (Alpaca or IBKR based on BROKER_PROVIDER env)
    let broker_provider = std::env::var("BROKER_PROVIDER").unwrap_or_else(|_| "alpaca".to_string());
    let broker_client: Option<Arc<dyn BrokerClient>> = match broker_provider.as_str() {
        "ibkr" | "interactive_brokers" => match ibkr_broker::IbkrClient::from_env() {
            Ok(client) => {
                if client.is_paper() {
                    tracing::info!("Broker: IBKR connected (Paper Mode)");
                } else {
                    tracing::warn!("Broker: IBKR connected (LIVE Mode)");
                }
                Some(Arc::new(client) as Arc<dyn BrokerClient>)
            }
            Err(e) => {
                tracing::warn!(
                    "IBKR broker not configured: {}. Trading features disabled.",
                    e
                );
                None
            }
            ...
            match AlpacaClient::from_env() {
                Ok(client) => {
                    ...
                    if !client.is_paper() {
                        ...
                        tracing::warn!(
                            "Broker: Alpaca connected to LIVE trading: {}",
                            client.base_url()
                        );
                    } else {
                        tracing::info!("Broker: Alpaca connected (Paper Trading Mode)");
                    }
                ...
                Err(e) => {
                    tracing::warn!(
                        "Alpaca broker not configured: {}. Trading features disabled.",
                        e
                    );
    ...
    // Initialize ML provider (PyO3 embedded if feature enabled, else HTTP)
    let ml_provider: Option<Arc<dyn ml_client::MLProvider>> = {
        #[cfg(feature = "embedded-ml")]
        {
            match ml_engine::EmbeddedMLEngine::initialize() {
                Ok(engine) => {
                    tracing::info!("ML provider: embedded PyO3 engine");
                    Some(Arc::new(engine) as Arc<dyn ml_client::MLProvider>)
                }
                Err(e) => {
                    tracing::warn!("PyO3 ML init failed: {e}, falling back to HTTP");
                    Some(Arc::new(ml_client::HttpMLProvider::new(
                        ml_client::MLClient::with_defaults(),
                    )) as Arc<dyn ml_client::MLProvider>)
                }
            }
        }
    ...
    // Initialize WebSocket broadcast (always available; WS clients connect optionally)
    let ws_broadcast = ws_routes::WsBroadcast::new();

    // Initialize notification service (email + Discord alerts)
    let notification = {
        let config = notification_service::NotificationConfig::from_env();
        Arc::new(notification_service::NotificationService::new(&config))
    };

    // Polygon WebSocket bridge (opt-in via POLYGON_WS_ENABLED=true)
    if std::env::var("POLYGON_WS_ENABLED")
        .map(|v| v.eq_ignore_ascii_case("true"))
        .unwrap_or(false)
    {
        let polygon_key = std::env::var("POLYGON_API_KEY").unwrap_or_default();
        let (polygon_ws, mut polygon_rx) =
            polygon_client::websocket::PolygonWebSocket::new(polygon_key);
        let ws_bridge = ws_broadcast.clone();
        tokio::spawn(async move { polygon_ws.run().await });
        tokio::spawn(async move {
            while let Ok(quote) = polygon_rx.recv().await {
                ws_bridge
                    .publish_quote(ws_routes::LiveQuote {
                        symbol: quote.symbol,
                        price: quote.price,
                        bid: quote.bid,
                        ask: quote.ask,
        ...
        tracing::info!("Polygon WebSocket streaming enabled");
    }

    // Alpaca WebSocket bridge for order updates (opt-in via ALPACA_WS_ENABLED=true)
    if std::env::var("ALPACA_WS_ENABLED")
        .map(|v| v.eq_ignore_ascii_case("true"))
        .unwrap_or(false)
    {
        ...
        let is_paper = std::env::var("ALPACA_BASE_URL")
            .map(|u| u.contains("paper"))
            .unwrap_or(true);
        let (alpaca_ws, mut alpaca_rx) =
            alpaca_broker::websocket::AlpacaWebSocket::new(alpaca_key, alpaca_secret, is_paper);
        tokio::spawn(async move { alpaca_ws.run().await });
        tokio::spawn(async move {
            while let Ok(update) = alpaca_rx.recv().await {
                tracing::info!(
                    "Alpaca order update: {} {} {} ({})",
                    update.event,
                    update.side,
        ...
        tracing::info!("Alpaca WebSocket order updates enabled");
    }
    ...
    let state = AppState {
        ...
        ml_provider,
        ws_broadcast: Some(ws_broadcast),
    ...
    // --- Startup environment validation ---
    {
        ...
        if api_keys.is_empty() {
            if require_auth {
                anyhow::bail!("REQUIRE_AUTH is set but API_KEYS is empty. Cannot start without authentication in production mode.");
            }
            ...
            tracing::info!(
                "API authentication enabled ({} key(s) configured)",
                api_keys.len()
            );
        }
        ...
        if state.broker_client.is_none() {
            tracing::warn!(
                "Trading features disabled (APCA_API_KEY_ID/APCA_API_SECRET_KEY not set)"
            );
        }
        if std::env::var("FINNHUB_API_KEY").is_err() {
            tracing::info!("FINNHUB_API_KEY not set — Finnhub news source disabled");
        }
        if std::env::var("LIVE_TRADING_KEY").is_err() && state.broker_client.is_some() {
            tracing::info!(
                "LIVE_TRADING_KEY not set — broker write endpoints disabled (safe default)"
            );
        }
        if std::env::var("DISCORD_WEBHOOK_URL").is_ok() || std::env::var("SMTP_HOST").is_ok() {
            tracing::info!("Notification channels configured");
        } else {
            tracing::info!(
                "No notification channels configured (set DISCORD_WEBHOOK_URL or SMTP_HOST)"
            );
        }
    ...
    let replenish_interval_ms = 60_000u64
        .checked_div(rate_limit_per_minute.max(1))
        .unwrap_or(1000);
    let burst = rate_limit_per_minute.min(200) as u32;

    let mut governor_builder = GovernorConfigBuilder::default().key_extractor(SmartIpKeyExtractor);
    ...
    // With Starter plan (unlimited API calls), this completes in ~1-2s.
    {
        ...
        tokio::spawn(async move {
            ...
            let (spy, tlt, gld, qqq, dia, iwm) = tokio::join!(
                warm_state
                    .orchestrator
                    .get_bars("SPY", Timeframe::Day1, 365),
                warm_state
                    .orchestrator
                    .get_bars("TLT", Timeframe::Day1, 365),
                warm_state
                    .orchestrator
                    .get_bars("GLD", Timeframe::Day1, 365),
                warm_state
                    .orchestrator
                    .get_bars("QQQ", Timeframe::Day1, 365),
                warm_state
                    .orchestrator
                    .get_bars("DIA", Timeframe::Day1, 365),
                warm_state
                    .orchestrator
                    .get_bars("IWM", Timeframe::Day1, 365),
            );
            let results = [
                ("SPY", spy),
                ("TLT", tlt),
            ...
            for (etf, result) in results {
                match result {
                    Ok(bars) => {
                        ...
                        warm_state
                            .etf_bar_cache
                            .insert(cache_key_90, (Utc::now(), subset));
    ...
    let app = app
        ...
        .merge(
            risk_routes::risk_routes().layer(middleware::from_fn_with_state(
                ip_allowlist.clone(),
            ...
            ServiceBuilder::new()
                .layer(HandleErrorLayer::new(|_: tower::BoxError| async {
                    (
                        ...
                        Json(ApiResponse::<()>::error("Request timeout".to_string())),
                    )
                }))
    ...
    // Shared PID so the shutdown path can kill the frontend process group
    let frontend_child_pid = Arc::new(std::sync::atomic::AtomicU32::new(0));

    // --- Bind the TCP listener BEFORE starting the frontend ---
    // This prevents orphaned gunicorn processes when the port is already in use.
    ...
    // For non-TLS, bind early so we fail fast before spawning child processes
    let listener = if !use_tls {
        match tokio::net::TcpListener::bind(addr).await {
            Ok(l) => {
                tracing::info!("🚀 API Server bound to http://{}", addr);
                Some(l)
            }
            Err(e) => {
                tracing::error!(
    ...
    let frontend_handle = if no_frontend {
        ...
        match python_manager::PythonManager::new() {
            Ok(mgr) => match mgr.start(frontend_child_pid.clone()) {
            ...
            Err(e) => {
                tracing::error!(
                    "Failed to initialize frontend manager: {}. Continuing API-only.",
                    e
                );
                None
            }
    ...
    // Helper: ensure frontend is cleaned up on any exit path
    let cleanup = |fh: Option<tokio::task::JoinHandle<()>>, pid: &std::sync::atomic::AtomicU32| {
        shutdown_frontend(fh, pid);
    };

    // Start server with graceful shutdown
    let serve_result = if use_tls {
        let cert_path = std::env::var("TLS_CERT_PATH").unwrap();
        let key_path = std::env::var("TLS_KEY_PATH").unwrap();
        ...
        let listener = listener.expect("listener must exist for non-TLS");
        axum::serve(
            listener,
    ...
    // Always clean up the frontend, whether the server exited normally or on error
    cleanup(frontend_handle, &frontend_child_pid);
...
#[utoipa::path(
    ...
    tag = "System",
    responses(
        (status = 200, description = "Prometheus exposition format metrics", content_type = "text/plain"),
    ),
)]
...
#[utoipa::path(
    ...
    tag = "System",
    responses(
        (status = 200, description = "JSON metrics for dashboard"),
    ),
)]
/// JSON metrics endpoint for dashboard
async fn metrics_json_endpoint(State(state): State<AppState>) -> impl IntoResponse {
    let m = &state.metrics;
    let buckets = [
        "<10ms", "<50ms", "<100ms", "<250ms", "<500ms", "<1000ms", ">=1000ms",
    ];
    let latency: serde_json::Value = buckets
        .iter()
        .enumerate()
        .map(|(i, label)| {
            (
                label.to_string(),
                serde_json::Value::from(m.latency_buckets[i].load(Ordering::Relaxed)),
            )
        })
...
#[utoipa::path(
    ...
    tag = "System",
    responses(
        (status = 200, description = "Service healthy or degraded"),
        (status = 503, description = "Critical dependency down"),
    ),
)]
async fn health_check(State(state): State<AppState>) -> impl IntoResponse {
        ...
        let db_ok = if let Some(pm) = state.portfolio_manager.as_ref() {
            matches!(
                tokio::time::timeout(check_timeout, async {
                    sqlx::query("SELECT 1").execute(pm.db().pool()).await
                })
                .await,
                Ok(Ok(_))
            )
        ...
        checks.insert(
            ...
            serde_json::to_value(DepCheck {
                ...
                error: if polygon_ok {
                    ...
                    Some("Polygon API unreachable".to_string())
                },
            })
            .unwrap(),
        );
    ...
    {
        let start = std::time::Instant::now();
        let alpaca_ok = if let Some(alpaca) = state.broker_client.as_ref() {
            matches!(
                tokio::time::timeout(check_timeout, alpaca.get_account()).await,
                Ok(Ok(_))
            )
        } else {
        ...
        let status_str = if state.broker_client.is_none() {
        ...
        checks.insert(
            ...
            serde_json::to_value(DepCheck {
                status: status_str,
                latency_ms: latency,
                error: if alpaca_ok || state.broker_client.is_none() {
                    None
                } else {
                    Some("Alpaca unreachable".to_string())
                },
            })
            .unwrap(),
        );
        ...
        checks.insert(
            ...
            serde_json::to_value(DepCheck {
                ...
                error: if ml_ok {
                    ...
                    Some("ML service unreachable".to_string())
                },
            })
            .unwrap(),
        );
...
#[utoipa::path(
    ...
    responses(
        (status = 200, description = "Full multi-engine analysis"),
...
#[utoipa::path(
    ...
    responses(
        (status = 200, description = "OHLCV bar data"),
        (status = 500, description = "Data fetch error"),
    ),
    security(("api_key" = []), ("bearer" = [])),
)]
async fn get_bars(
    ...
    let bars = state
        .orchestrator
        .get_bars(&symbol, timeframe, days)
        .await?;
...
#[utoipa::path(
    ...
    responses(
        (status = 200, description = "Ticker details from Polygon"),
        (status = 500, description = "Data fetch error"),
    ),
    security(("api_key" = []), ("bearer" = [])),
)]
...
#[utoipa::path(
    ...
    tag = "Screener",
    params(
        ("universe" = Option<String>, Query, description = "Stock universe: popular, tech, bluechip, or comma-separated symbols"),
        ("min_confidence" = Option<f64>, Query, description = "Minimum confidence threshold (0.0-1.0, default: 0.5)"),
        ("min_signal" = Option<i32>, Query, description = "Minimum signal strength (-3 to 3, default: 0)"),
        ("limit" = Option<usize>, Query, description = "Max results (default: 10)"),
    ),
    responses(
        (status = 200, description = "Screened stock suggestions"),
        (status = 500, description = "Screening error"),
    ),
    security(("api_key" = []), ("bearer" = [])),
)]
...
async fn validate_analysis(
    ...
    let our_rsi = analysis
        .technical
        .as_ref()
        .and_then(|t| t.metrics.get("rsi"))
        .and_then(|v| v.as_f64());

    let our_macd = analysis.technical.as_ref().and_then(|t| {
    ...
    // Perform comparison
    let comparison = comparison_engine
        .full_comparison(
            &symbol,
            our_rsi,
            our_macd,
...
#[utoipa::path(
    ...
    params(
        ("symbol" = String, Path, description = "Stock ticker symbol"),
        ("days" = Option<i64>, Query, description = "Lookback period in days (default: 365)"),
    ),
    responses(
        (status = 200, description = "Point-in-time backtest results with equity curve, trades, and metrics"),
        (status = 500, description = "Backtest error"),
    ),
    security(("api_key" = []), ("bearer" = [])),
)]
async fn backtest_symbol(
    ...
    tracing::info!(
        "Running point-in-time backtest for {} ({} days)",
        symbol,
        days
    );
    ...
    if bars.len() < 50 {
        return Err(anyhow::anyhow!(
            "Not enough historical data for backtesting (need >= 50 bars)"
        )
        .into());
    }
...
impl IntoResponse for AppError {
    fn into_response(self) -> Response {
        if self.status == StatusCode::INTERNAL_SERVER_ERROR {
            tracing::error!("API error: {:#}", self.inner);
        } else {
            tracing::warn!("API error ({}): {:#}", self.status.as_u16(), self.inner);
        }

        // Report only 500s to Sentry
        #[cfg(feature = "sentry")]
        {
            if self.status == StatusCode::INTERNAL_SERVER_ERROR {
                sentry::capture_error(&*self.inner);
            }
        }
        ...
        let is_production = std::env::var("RUST_ENV")
            .map(|v| v == "production")
            .unwrap_or(false);
        let message = if is_production && self.status == StatusCode::INTERNAL_SERVER_ERROR {
